...
void ClampSwitch(Mold_typ * pMold, Clamp_Fix_typ * pClampFix,Clamp_Para_typ * pClampPara,SPC_Mold_typ * pSPC)
{
	...
	switch ( pMold->Clamp.Step )
	{
		...
		case 10400:	/* open   */
			...
			/*if(1 == gMachineInfo.MachineReset && 1 == gMachineInfo.Manual)*/  /* �Զ���λ */
   ...
   /*********************************************************************************
	...
	switch ( pMold->Clamp.Step )
	{
		...
		case 12200:		/* delay ok ΢�_ģ �_�����D��λ��*/
			...
			if( 1 == pMold->Option.SubMold)
			{
				if(pMold->Clamp.mmAs > pMold->Clamp.S_SubMoldOut - 0.1)  // ΢�_ģ
				{		     
					pMold->Clamp.v_set  = 0;
					pMold->Clamp.p_set  = 0;
			...
			{
				if(pMold->Clamp.mmAs > pMold->Clamp.S_OpnSlow - 0.1)	 // ���D��
				{		     
					pMold->Clamp.v_set  = 0;
					pMold->Clamp.p_set  = 0;
					pMold->Clamp.Step = 12300;
...
//			pMold->ValveOut.ClampOpnRelease = 0;
		...
		case 10300:	/* valve open delay  */
			if(0 == pMold->Clamp.OpnHWSwitch)
			{
				pMold->Clamp.LimitTimer.IN = 1;
				if(1 == gMachineInfo.MachineReset)
					/*if(1 == gMachineInfo.MachineReset && 1 == gMachineInfo.Manual)*/  /* �Ԅӏ͚w  */
				{
	...
	if(pMold->Clamp.Step < 3000 && pMold->Clamp.Step > 200) 	/* clamp close  */
	{
		...
		if(1 == gMachineInfo.Auto && 1 == gMachineFix.Option.bCoolPinUpNextCycClampCls)   /* ����ģʽ  "���һѭ�h�Pģ���Dһ��"  */
		{
	...
	if (pClampFix->eValveType == VALVE_PROP)
	{
		...
		if(5000 == pMold->Clamp.Step || 15000 == pMold->Clamp.Step || 8500 == pMold->Clamp.Step)
		{
			if(bClampPropValveOutputInverse == 0)
			{
				pMold->Clamp.aoPropValve = pMold->Clamp.v_set * 32767/100;   /* ��ѹ���  0 -- 100 ---> 0.0 -- 10.0v*/
			}
			else
			{
				pMold->Clamp.aoPropValve = -(pMold->Clamp.v_set * 32767/100);   /* ��ѹ���  0 -- 100 ---> 0.0 -- 10.0v*/
...
//			pMold->TimeDis.ClampClsAlarmTime = pMold->Clamp.LimitTimer.ET;
		...
		case 12100:/*UnLock Process   �_ģ����*/			
			...
			if( 1 == pMold->Option.SubMold)
			{
				pMold->Clamp.Step = 12130; //΢�_���t+ ΢�_
			}
			else
			{
				pMold->Clamp.Step = 12300; //΢�_���t+ ΢�_ ���^
			}
		...
		case 12150:
			...
			pMold->Clamp.vProfGen.s_end = pMold->Clamp.S_SubMoldOut; // ΢�_ģ
